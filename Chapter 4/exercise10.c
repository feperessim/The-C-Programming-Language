...
/* Exercise 4-10. An alternate organization uses getline to read an entire input line; this makes getch and ungetch unnecessary. Revise the
...
int getop(char s[MAXLINE])
{
        int i, c;
	
	if (line[j] == '\0') {
		if ((lgetline(line, MAXLINE)) == 0)
			return EOF;
	else
		j = 0;
	}
